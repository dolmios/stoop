...
<Icon name="Code" size={24} style={{ display: "inline", verticalAlign: "middle", marginRight: "8px" }} /> Create styled components with type-safe props and variants
...
**Merging order**: Base styles → Variants → CSS props → `css` prop (later styles override earlier ones)

## CSS Properties as Props

CSS properties can be passed as props and are automatically converted to CSS styles:
...
CSS properties passed as props are merged before the `css` prop, allowing for prop-based overrides.
