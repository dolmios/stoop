...
2. **Use `preloadTheme()` to prevent FOUC** - Prevents FOUC when loading saved theme preferences from localStorage
3. **Include theme variables** - `getCssText()` automatically includes CSS variables for all configured themes
4. **No theme parameter needed** - `getCssText()` always includes all themes, so no need to pass a theme name
