...
- Update `data-theme` attribute if different from default
- Handle theme switching via `useTheme` hook
...
1. **SSR**: HTML renders with `data-theme="light"` → CSS variables apply immediately
2. **Hydration**: Provider mounts and checks localStorage
3. **Theme Sync**: If stored theme differs, Provider updates `data-theme` attribute
4. **Result**: No FOUC, smooth theme transitions
