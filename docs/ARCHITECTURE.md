...
Stoop is a CSS-in-JS library that compiles CSS objects into CSS classes at runtime. It uses a hash-based class generation system, CSS variables for theme tokens, and injects all styles into a single stylesheet in the document head for optimal performance.
...
- Returns configured functions (`styled`, `css`, `createTheme`, `globalCss`)
...
2. Check if value contains "$"
3. Extract token: "primary" (shorthand) or "colors.primary" (full path)
4. Resolve token: searches all categories for "primary", finds "colors.primary"
5. Convert to CSS variable: "var(--colors-primary)"
6. CSS variables are injected once in :root
7. Theme switching updates CSS variables, not CSS classes
```
...
1. **CSS Variables**: Theme tokens use CSS variables, enabling instant theme switching without recompiling CSS
2. **Single Stylesheet**: All CSS is injected into one `<style>` tag, reducing DOM overhead
3. **Style Caching**: Styles are cached by CSS string to avoid duplicate injection
4. **Memoization**: Styled components use optimized `useMemo` hooks with minimal dependencies (`variantKey`, `cssObject`) to cache class names efficiently
5. **Hash-based Classes**: Fast class name generation using hash functions
6. **Deduplication**: CSS rules are deduplicated to prevent redundant styles
7. **Efficient Theme Updates**: CSS variables update in-place when theme changes
8. **Token Index Cache**: Theme token lookups use WeakMap-based index for O(1) performance instead of O(n) recursive search
9. **Modular Injection**: CSS injection split into focused modules (browser, SSR, dedup) for better maintainability
...
Stoop uses CSS variables (CSS custom properties) for theme tokens, which provides several benefits:

1. **Instant Theme Switching**: Changing themes updates CSS variables, not CSS classes
2. **No Recompilation**: Components don't need to recompile when theme changes
3. **Better Performance**: Browser handles theme updates natively
4. **Smaller Bundle**: CSS variables reduce runtime overhead

### How It Works

1. Theme tokens (`$primary` or `$colors.primary`) are converted to CSS variables (`var(--colors-primary)`)
2. CSS variables are injected once in `:root` block
