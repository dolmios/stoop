...
Global styles are configured in the `globalCss` property of your Stoop config (not in the theme object). The Provider automatically injects them when `themes` config is provided, so no manual setup is required!

If you're not using the `themes` config, you can call `globalCss()` manually:

```tsx
const { globalCss } = createStoop({
  theme: lightTheme,
  globalCss: {
...
// If not using Provider, inject manually:
globalCss({
  "*": { margin: 0, padding: 0, boxSizing: "border-box" },
  body: {
    fontFamily: "system-ui, sans-serif",
...
// Preload all themes BEFORE React renders - prevents FOUC
try {
